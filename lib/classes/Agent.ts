...
export class Agent {
	...
	constructor(clientEvents: ClientEvents) {
		...
		this.clientEvents.onGroupChatAgentListUpdate.subscribe(
			(event: GroupChatSessionAgentListEvent) => {
	...
	setInitialAppearance() {
		...
		circuit
			.waitForMessage<AgentWearablesUpdateMessage>(
				Message.AgentWearablesUpdate,
				10000
			)
			.then((wearables: AgentWearablesUpdateMessage) => {
				if (
					!this.wearables ||
					wearables.AgentData.SerialNum > this.wearables.serialNumber
				) {
					this.wearables = {
				...
				Object.keys(this.inventory.main.skeleton).forEach(uuid => {
					...
					if (folder.typeDefault === FolderType.CurrentOutfit) {
						...
						this.currentRegion.caps
							.capsRequestXML('FetchInventoryDescendents2', requestedFolders)
							.then((folderContents: any) => {
								const currentOutfitFolderContents =
									folderContents['folders'][0]['items'];
								const wornObjects = this.currentRegion.objects.getObjectsByParent(
									this.localID
								);
										...
										wornObjects.forEach((obj: GameObject) => {
											if (obj.hasNameValueEntry('AttachItemID')) {
												if (
													...
													obj.getNameValueEntry('AttachItemID')
												) {
													found = true;
												}
											}
